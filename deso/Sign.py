...
class SignTransaction:
    ...
    def get_hmac(self, key, data):
        return hmac.new(key, data, hashlib.sha256).digest()
    
    
    def hmac_drbg(self, entropy, string):
        material = entropy + string
        K = b"\x00" * 32
        V = b"\x01" * 32
    
        K = self.get_hmac(K, V + b"\x00" + material)
        V = self.get_hmac(K, V)
        K = self.get_hmac(K, V + b"\x01" + material)
        V = self.get_hmac(K, V)
    ...
    def point_add(self, point1, point2):
        ...
        if x1 == x2:
            m = (3 * x1 * x1) * self.inverse_mod(2 * y1, p)
        else:
            m = (y1 - y2) * self.inverse_mod(x1 - x2, p)
    ...
    def to_DER(self, r, s):  # Signature to DER format
        r = bytes.fromhex(r)
        s = bytes.fromhex(s)
        if r[0] >= 0x80:
    ...
    def Sign_Transaction(self, seedHex, TransactionHex):
        ...
        drbg = self.hmac_drbg(entropy=bytes.fromhex(seedHex), string=s256)
        k = int.from_bytes(drbg, "big")
        kp = self.scalar_mult(k, self.g)
        kpX = kp[0]
        r = kpX % self.n
        s = self.inverse_mod(k, self.n) * (r * int(seedHex, 16) + int(s256.hex(), 16))
        s = s % self.n
        # Enforce low-s -> credit to @Nathanwells 
        if s > self.n // 2:
            s = self.n - s
        signature = self.to_DER(self.hexify(r), self.hexify(s))
        # Added for Balance Model fork
        signatureIndex = int(self.getTransactionIndex(TransactionHex))
